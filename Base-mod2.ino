...
void setup() {
  ...
  wait(30000); // wait for preperation after setup check
}
...
void loop() {
  ...
  if (vStop == 1) // search for the marker if not detected with in the indicated range
  {
    finalSearch();
  }
  else
...
float driveStraight(long i) 
{
  ...
  while ((distance += totalDistance) < i)
  {
      ...
      */
      ...
      dist = ultraSonic();
      if (dist < 10)   // getVoltage(A0) < 1 || getVoltage(A1) < 1 
      {
        //ReactObstacles();
        LeftEncoderCount = 0;
        RightEncoderCount = 0;
